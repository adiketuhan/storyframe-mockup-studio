  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Tulis naskah cerita dalam format teks, sistem regex akan otomatis menyusun seluruh slide & karakter!
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              title="Panduan Syntax Naskah"
            >
              <HelpCircle className="w-4 h-4 text-indigo-400" />
              <span className="hidden sm:inline">Panduan Syntax</span>
            </button>
        ...
        {/* Modal Body Grid */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Main Script Textarea (Left 2 cols) */}
          <div className="lg:col-span-2 flex flex-col space-y-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1.5 overflow-x-auto">
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg text-indigo-300 flex items-center space-x-1"
                >
                  <Sparkles className="w-3 h-3" />
                  <span>Contoh Naskah Misteri</span>
                </button>

                <button
                  type="button"
                  onClick={() => setScriptText('[JUDUL]: Judul Cerita Baru\n[WAKTU_MULAI]: 21:00\n\n[SLIDE 1 - WHATSAPP]\n[KONTAK]: Sarah\n[STATUS]: online\n\nTHEM: Hai, kamu dimana?\nME: Lagi di jalan pulang.')}
                  className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg text-slate-300"
                >
                  Template Polos
                </button>
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg text-slate-400 hover:text-red-300"
                >
                  Bersihkan
                </button>
              </div>
            ...
            <div className="relative flex-1 min-h-[300px] lg:min-h-[420px]">
              <textarea
                ...
                placeholder="Tulis naskah cerita di sini..."
                className="w-full h-full p-4 bg-slate-950 border border-slate-800 rounded-2xl text-slate-200 font-mono text-xs leading-relaxed focus:outline-none focus:border-indigo-500 resize-none selection:bg-indigo-500/30"
                spellCheck={false}
              />
            </div>
          </div>

          {/* Right Sidebar: Live Detection & Cheat Sheet */}
          <div className="space-y-4 flex flex-col">
            {/* Live Parsing Statistics */}
            ...
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-3.5 space-y-2 flex-1 text-xs">
              <div className="flex items-center space-x-1.5 text-slate-300 font-bold text-xs">
                <BookOpen className="w-3.5 h-3.5 text-indigo-400" />
                <span>Format Kata Kunci (Regex):</span>
              </div>
              <ul className="space-y-1.5 text-[11px] text-slate-400">
                <li><code className="text-indigo-300 bg-slate-900 px-1 py-0.5 rounded">[SLIDE 1 - WHATSAPP]</code></li>
                <li><code className="text-indigo-300 bg-slate-900 px-1 py-0.5 rounded">[SLIDE 2 - IG_FEED]</code></li>
                <li><code className="text-indigo-300 bg-slate-900 px-1 py-0.5 rounded">[SLIDE 3 - TWITTER]</code></li>
                <li><code className="text-emerald-300 bg-slate-900 px-1 py-0.5 rounded">THEM: teks</code> / <code className="text-emerald-300 bg-slate-900 px-1 py-0.5 rounded">ME: teks</code></li>
                <li><code className="text-amber-300 bg-slate-900 px-1 py-0.5 rounded">(vn: 0:14)</code> Voice Note</li>
                <li><code className="text-red-300 bg-slate-900 px-1 py-0.5 rounded">(deleted)</code> Pesan Dihapus</li>
                <li><code className="text-sky-300 bg-slate-900 px-1 py-0.5 rounded">[NOTIFIKASI]: WA | Judul | Pesan</code></li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 py-4 border-t border-slate-800 flex items-center justify-between bg-slate-950/80">
          <span className="text-xs text-slate-500 hidden sm:inline">
            Setelah digenerate, Anda tetap bisa mengedit tiap slide secara visual.
          </span>
